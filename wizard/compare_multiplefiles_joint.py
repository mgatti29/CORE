...
def compare(photo_z_columns,true_column,resampling,label_output,path_wz_samples,path_datasets,priors,tomo_bins,sigma='ALL',add_noise=False,shift_pz_1= None, model_kwargs = {'z0': 0.5},
    ...
    # plot *********************
    plot(pz,wz,wz_corrected,wz_cov,wz_cov_corrected ,label_output,compare_path,cov_mode,shift,errp,errm,nphoto,amplitudes)
...
# ****************************************************************************************
def plot(pz,wz,wz_corrected,wz_cov,wz_cov_corrected ,label_save,output,cov_mode,shift,errp,errm,nphoto,amplitudes):
    ...
    #    plt.errorbar(wz[i]['z_centers'],wz_corrected[i]['wz'],err_corrected,fmt='o',color='blue',label='clustz corrected')
        ...
        if nphoto==1:
            residual=-np.mean(pz[i][str(0)]['z_corrected'])+np.mean(pz[i]['z_true_residual'])
