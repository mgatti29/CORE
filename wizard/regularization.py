    ...
    #for file in os.listdir('./output_dndz/best_Nz/'):
        ...
        if ('gaussian' not in file):
            ...
            if fit=='gaussian_process' :
                ...
                plot2(z,z_edges,zp_t_TOT,Nz,Nz_jack,N,label_save,output,jk_r,only_diagonal,cov,err,prior_gaussian_process)
            else:
                ...
                NN[:,1:]=Nz_jack
                output='./output_dndz/best_Nz/'
                plot(z,z_edges,zp_t_TOT,NN,N,label_save,output,jk_r,False, False,False,only_diagonal,False)
#*************************************************************************
#                 plotting & saving
#*************************************************************************

def plot2(z,z_bin,zp_t_TOT,Nz,Nz_jack,N,label_save,output,jk_r,only_diagonal,cov,err,prior_gaussian_process):


    label_save=label_save.replace(".h5", "")
    with Silence(stdout='./output_dndz/best_Nz/gaussian_log{0}.txt'.format(label_save), mode='w'):
