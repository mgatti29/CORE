...
def dndz(methods=['Menard_physical_scales'],
        ...
        for nnn in range(len(Nbins)):
                ...
                for thetmax in range(interval_bins,Nbins[nnn]+1,step):
                    for thetmin in range(0,thetmax-interval_bins+1,step):
                        ...
                        correlation_optimized=optimize(correlation,methods,N,reference_bins_interval,bias_correction_Menard,bias_correction_Newman,bias_correction_Schmidt,weight_variance,
                        ...
                        # Compute N(z) *************************************************
                        Nz_tomo,BNz_tomo=compute_Nz(correlation_optimized,jk_r,methods,bias_correction_Menard,bias_correction_Newman,bias_correction_Schmidt
                        ...
                        for method in Nz.keys():
                            ...
                            if resampling_pairs:
                                ...
                                resp=''
                            label_save='Nz_{0}_{1}_{2}_{3}_{4}_{5}_{6}_{7}_{8}_{9}'.format(method,label_weight,label_bias,labeladd,thetmin,thetmax,Nbins[nnn],resampling,resp,jk_r)
                            label_save1='BNz_{0}_{1}_{2}_{3}_{4}_{5}_{6}_{7}_{8}_{9}'.format(method,label_weight,label_bias,labeladd,thetmin,thetmax,Nbins[nnn],resampling,resp,jk_r)
    ...
    if regularization :
        regularization_routine(best_params,reference_bins_interval['z'],reference_bins_interval['z_edges'],
